...
import { ArrowLeft, Check, X, Calendar, ChevronDown } from 'lucide-react-native';
...
const AddTransaction: React.FC<NavigationProps<'AddTransaction'>> = ({ navigation, route }) => {
  ...
  const setupEditMode = (transaction: Transaction) => {
    ...
    if (transaction.cuenta) {
      // La cuenta se establecerá cuando se carguen las cuentas en loadAccounts
      const account = accounts.find(acc => acc.nombre === transaction.cuenta);
      if (account) {
  ...
  const handleSave = async () => {
    ...
    if (selectedSubcategory && selectedSubcategory.categoriaPadreId !== selectedCategory?.id) {
      Alert.alert('Error', 'La subcategoría seleccionada no pertenece a la categoría principal.');
      return;
    }
